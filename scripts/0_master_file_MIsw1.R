...
site <- 'MI_FM1' # site abbreviation that may be used for file naming conventions
...
site_tz <- 'Etc/GMT+5' # all data should be converted (or called into R) to the same time zone. 
...
######## BMP timeline ############
start_date <- as.POSIXct('2012-05-01 12:00:00') # YYYY-MM-DD HH:MM:SS date/time of study start
                             # This can be the first second of the day on the date of the first sampling event
bmp_date <- as.POSIXct('2014-11-01 12:00:00') # YYYY-MM-DD HH:MM:SS date/time of BMP implementation. All events before
                           # this date will be considered 'before', all events after this 
                           # date will be considered 'after'
end_date <- as.POSIXct('2017-09-30 23:59:59') # YYYY-MM-DD HH:MM:SS date/time of study end

######## storm event data #############
## set wq data parameters
wq_file <- 'prepped_wq_bystorm.csv' # file name (with .csv extention) where event-level water quality data are stored

# for setting concentrations and loads, only include the variables you want to 
...
concentrations <- 'mg_L' # this can either be a vector of verbatim constituent concentrations
...
clean_names <- c('SS (mg/L)', 'Chloride (mg/L)', 
                 'NO2 + NO3 (mg/L)', 'Ammonium (mg/L)', 
                 'TKN (mg/L)', 'Orthophosphate (mg/L)', 
                 'TP (mg/L)', 'TN  (mg/L)', 
                 'Org N (mg/L)', 'SS load (pounds)', 'Chloride load (pounds)', 
...
event_over_thaw <- 'frozen' # For subevents that span a thaw, you may have to decide whether to split the events or combine them.
...
######## rain data ###########
rain_site <- '425443083472501' # site number for rain gage if pulling from NWIS, 'NA' if providing data file
rain_file <- '' # filename of rain file, 'NA' if pulling from NWIS. Should be a csv and include ".csv" at end of file name.

######## field activity data #########
activity_file <- 'FarmActivityData.csv' # filename of field activity log and should include .csv extension
...
discharge_site_no <- '041482663' # if not providing a discharge file, the USGS stream gage site to pull discharge data from
...
######## weather data ##############
weather_file <- 'Soil_Cond.csv' # filename (ending in .csv) of daily weather data from a nearby met station to calculate storm
...
noaa_site <- 'USW00014826' # nearest NOAA met station site number. Can use both weather_file and noaa_site if 
                # weather_file does not contain snow depth. 

noaakey <- '' # set NOAA access key. See SOP for more details.

other_weather_vars <- c('SOIL_Moist_perc', 'SOIL_Temp_F') # if you have other daily weather/site characteristic data in your file (e.g., soil temp) and want to 
